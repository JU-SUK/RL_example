...
from environment import ContinuousCartPoleEnv
# 정책 신경망과 가치 신경망 생성
...
class ContinuousA2CAgent:
    def __init__(self, action_size, max_action):
        ...
        # 액터-크리틱 hyperparameter
        self.discount_factor = 0.99
        self.learning_rate = 0.001

        # 정책신경망과 가치신경망 생성
        self.model = ContinuousA2C(self.action_size)
        # 최적화 알고리즘 설명, 미분값이 너무 커지는 현상을 막기 위해 clipnorm 설정
...
if __name__ == "__main__":
    # cartpole-v1
    # gym.envs.register(id = 'CartPoleContinuous-v0', entry_point= 'env:ContinuousCartPoleEnv', max_episode_steps=500, reward_threshold=475.0)
    ...
    for e in range(num_episode):
        ...
        while not done:
            ...
            if done:
                ...
                print("episode : {:.3f} | score avg : {:.3f} | loss : {:.3f} | sigma : {:.3f}".format(e, score_avg, np.mean(loss_list), np.mean(sigma)))
                scores.append(score_avg)
                episodes.append(e)
                pylab.plot(episodes, scores, 'b')
                pylab.xlabel('episode')
                pylab.ylabel('average_score')
