...
class Actor(Model):
    ...
    def call(self, state):
        ...
        # 평균값을 [-action bound, action_bound] 범위로 조정
    ...
    def train(self, max_episode_num):
        ...
        for ep in range(int(max_episode_num)):
            ...
            while not done:
                ...
                # 액터 신경망 업데이트
                self.actor_learn(tf.convert_to_tensor(states, dtype=tf.float32), tf.convert_to_tensor(actions, dtype=tf.float32), tf.convert_to_tensor(advantages, dtype=tf.float32))
